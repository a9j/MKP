...
select pg_temp.want('anon sees public settings only', (select count(*) from site_settings), 21);
...
select pg_temp.want('admin sees all settings', (select count(*) from site_settings), 22);
